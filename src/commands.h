...
public:
  ...
  enum commands {
    ...
    vuUpdate = 0x04,      //pc: sends mic L/R levels (0-255 each) on a loop; fire-and-forget, no ack needed
  };

  void actionUpdateCommand(byte (&buffdataIn)[5], int actionNumber) {
    buffdataIn[0] = header;
  ...
  void discoveryResponseCommand(byte (&buffdataIn)[5]) {
    buffdataIn[0] = header;
  ...
  void acknowledgeCommand(byte (&buffdataIn)[5]) {
    buffdataIn[0] = header;
  ...
  void disconnectCommand(byte (&buffdataIn)[5]) {
    buffdataIn[0] = header;
